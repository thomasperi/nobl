...
const factor = 10; // How many durations to wait between each sampling "frame"
...
test('default options, one sample', async () => {
	...
	assert.equal(nobl.doing, false);

	const promise = nobl.do(function* () {
		const now = Date.now();
		setTimeout(() => {
			sample1 = count;
			assert.equal(nobl.doing, true);
		}, 50);
